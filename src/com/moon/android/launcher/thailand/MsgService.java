...
import com.moon.android.launcher.thailand.database.RegionLimitDAO;
...
import com.moon.android.launcher.thailand.model.STBInfo;
import com.moon.android.launcher.thailand.model.UpdateData;
import com.moon.android.launcher.thailand.model.UserMsg;
...
import com.moon.android.launcher.thailand.util.VersionUtil;
		...
		if(!isBlank(requestResult)){
			try{  
				...
				if(null != regionLimit){
					...
					Declare.VALIDITY_TIME = regionLimit.getEnd_time();
					...
					//end
					logger.i("Region limit status = " + code);
					if(null != regionLimit.getEnd_time())
						ValiditySharepreference.setHasRead(getApplicationContext(), regionLimit.getEnd_time());
...
//							}
		...
		mTimerLauncherMsg.schedule(new TimerTask() {
			...
			public void run() {
				...
				try{
					if(list.size() > 0 && !isBlank(list.get(0).getBody())){
						Declare.listLauncherMsg = list;
						Intent intent = new Intent();
...
//	}
	...
	private List<LauncherMsg> loadLauncherMsg() {
		...
		for(LauncherMsg msg : list){
			logger.i("Scroll msg : " + msg.getTitle() + " " + msg.getBody());
		}
		return list;
	}
