...
@SuppressLint("NewApi")
public class IndexGrid extends LinearLayout implements OnKeyListener{
	
	public static final String TAG = "AppsView";
	...
	public IndexGrid(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		mContext = context;
		View view = LayoutInflater.from(context).inflate(R.layout.index_view, this);
	...
	private void getApps() {
		...
		//list.add(new NavItem(R.drawable.icon_t_setting,R.string.app_and_set,true,"",HomeActivity.class));
	//	mListAppInfo.add(new CustomAppInfo("11",SYS_SETTING,SYS_SETTING_AC,getDrawable(mContext,R.drawable.app_setting),true));
	//	one.icon=R.drawable.icon_t_setting;
//		mListAppInfo.addAll(Constant.getDefaultApp(mContext));
	//	if(!Configs.isShowSystemApp())
		//	mListAppInfo.addAll(Constant.getSysApp(mContext));
		mListAppInfo = new ArrayList<CustomAppInfo>();
		mListAppInfo.addAll(Constant.getDefaultApp(mContext));
		List<CustomAppInfo> userInstall  = AppUtils.loadAppFilter(mContext,false);
	...
	public IndexGrid(Context context) {
		this(context,null);
	}
	...
	private BroadcastReceiver appOperateReceiver = new BroadcastReceiver(){
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(!Configs.isShowSystemApp())
				mListAppInfo.addAll(Constant.getSysApp(mContext));
			List<CustomAppInfo> userInstall  = AppUtils.loadApplications(mContext,Configs.isShowSystemApp());
			mListAppInfo.addAll(userInstall);
			setAdapter(mListAppInfo);*/
...
//			startActivity(appInfo.pkgName);
			try {
				mContext.startActivity(appInfo.intent);
			} catch (Exception e) {
				// TODO: handle exception
				Toast.makeText(mContext.getApplicationContext(),mContext.getApplicationContext().getResources().getString(R.string.error_open_menu),
		...
		if (v == mGridApps && event.getAction() == KeyEvent.ACTION_DOWN) {
			...
			case KeyEvent.KEYCODE_DPAD_RIGHT: 
				...
				if (gridSelection + (mAppNumPerPage * (mCurrentPage-1)) + 1 == mListAppInfo.size()){
					...
					//int nextSelection = gridSelection - (mColumnNum - 1);
				//	Log.d("dd","pager2:"+mCurrentPage+"---grid:"+gridSelection+"----Colum:"+mColumnNum+"---next:"+nextSelection);
				//	mGridApps.setSelection(nextSelection);
				//	mGridApps.setSelection(nextSelection);
					return true;
					//翻到下一页
...
//						return false;
	...
	public static class AppsAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup arg2) {
			...
			if(null == convertView){
				holder = new Holder();
				convertView = mInflater.inflate(R.layout.indexapp_item, null);
